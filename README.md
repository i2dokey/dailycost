...
<div id="vMain" class="view show box">
    <div class="stat-box">
        <div class="stat-i"><span id="allCost" class="stat-v">0.00</span><span class="stat-l">总支出</span></div>
        <div style="width:1px;background:rgba(255,255,255,.3);margin:0 10px"></div>
        <div class="stat-i"><span id="allDay" class="stat-v">0.00</span><span class="stat-l">日均成本</span></div>
    </div>
    <div id="list" onclick="hideCtx()"></div>
    <div class="fab-group">
        <div class="fab" style="background:var(--g)" onclick="modSync(1)">⇄</div>
        <div class="fab" style="background:var(--t)" onclick="mod(1)">+</div>
    </div>
</div>

<div id="vCar" class="view box">
    <div class="car-h" onclick="swV(0)">‹ <span id="curCarN">返回</span></div>
    <div class="tabs">
        <div class="tab act" onclick="swT(0)" id="tb0">加油</div>
        <div class="tab" onclick="swT(1)" id="tb1">保养</div>
        <div class="tab" onclick="swT(2)" id="tb2">数据</div>
    </div>
    <div id="cv0" class="view show">
        <div class="stat-box bg-b"><div class="stat-i"><span id="cL100" class="stat-v">--</span><span class="stat-l">平均 L/100</span></div><div class="stat-i"><span id="cPerKm" class="stat-v">--</span><span class="stat-l">油费 /km</span></div></div>
        <div class="card">
            <div class="inp-w"><input type="number" id="cOdo" placeholder="当前里程 (km)"></div>
            <div class="inp-w"><input type="number" id="cCost" placeholder="金额 (元)"></div>
            <div class="inp-w"><input type="number" id="cVol" placeholder="油量 (L)"></div>
            <button id="btnF" class="btn btn-p" onclick="addF()">记一笔</button>
            <div id="tipF" style="text-align:center;font-size:12px;color:var(--s);margin-top:5px;display:none"><a href="javascript:rstF()" style="color:var(--r)">取消修改</a></div>
        </div>
        <div id="fList"></div>
    </div>
    <div id="cv1" class="view">
        <div class="stat-box bg-o"><div class="stat-i"><span id="mTot" class="stat-v">0.00</span><span class="stat-l">保养总花费</span></div></div>
        <div class="card">
            <div class="inp-w"><input type="text" id="mN" placeholder="项目名称"></div>
            <div class="inp-w"><input type="number" id="mC" placeholder="费用 (元)"></div>
            <button id="btnM" class="btn btn-o" onclick="addM()">记保养</button>
            <div id="tipM" style="text-align:center;font-size:12px;color:var(--s);margin-top:5px;display:none"><a href="javascript:rstM()" style="color:var(--r)">取消修改</a></div>
        </div>
        <div id="mList"></div>
    </div>
    <div id="cv2" class="view">
        <div class="card">
            <label style="font-size:12px;color:#888">购车费用/日期</label>
            <div class="inp-w"><input type="number" id="carP" onchange="upCP()" placeholder="车价"></div>
            <div class="inp-w"><input type="date" id="carD" onchange="upCP()"></div>
        </div>
        <div class="card">
            <div class="item-row"><span>🚙 购车</span><span id="dCarP" style="font-weight:700;margin-left:auto">0.00</span></div>
            <div class="item-row"><span>⛽ 油费</span><span id="dCarF" style="font-weight:700;margin-left:auto">0.00</span></div>
            <div class="item-row"><span>🛠️ 保养</span><span id="dCarM" style="font-weight:700;margin-left:auto">0.00</span></div>
            <div style="border-top:1px dashed #eee;margin:10px 0"></div>
            <div class="item-row"><span style="font-weight:700">总支出</span><span id="dCarT" style="color:var(--r);font-weight:700;margin-left:auto">0.00</span></div>
            <div class="item-row"><span>日均</span><span id="dCarDay" style="color:var(--p);font-weight:700;margin-left:auto">0.00</span></div>
        </div>
    </div>
</div>
...
<div id="modal" class="modal">
    <div class="m-cnt">
        <h3>添加资产</h3><br>
        <input type="text" id="nName" class="inp" placeholder="名称">
        <input type="number" id="nCost" class="inp" placeholder="金额 (元)">
        <input type="date" id="nDate" class="inp">
        <div class="file-area">
            <span id="fTxt">📷 上传照片</span>
            <input type="file" accept="image/*" onchange="onImg(this)" class="file-btn-real">
        </div>
        <div style="margin-bottom:10px;font-size:14px;color:var(--s)"><input type="checkbox" id="isCar"> 是汽车 (启用油耗)</div>
        <button id="btnAdd" class="btn btn-p" onclick="addIt()">确定</button>
        <button class="btn" style="background:#eee" onclick="mod(0)">取消</button>
    </div>
</div>
...
<div id="mSync" class="modal">
    <div class="m-cnt">
        <h3>数据同步</h3>
        <textarea id="syncCode" class="code-box" placeholder="数据码..."></textarea>
        <button class="btn btn-p" onclick="expD()">📤 导出本机数据</button>
        <button class="btn btn-g" onclick="impD()">📥 导入数据</button>
        <button class="btn" style="background:#eee" onclick="modSync(0)">关闭</button>
    </div>
</div>
...
<div id="vImg" class="modal">
    ...
    <div style="margin-top:20px;display:flex;gap:15px">
        <button class="btn btn-p" style="width:auto;padding:10px 25px;margin:0" onclick="modImg()">🖼️ 更换图片</button>
        <button class="btn" style="width:auto;padding:10px 25px;background:#333;color:#fff;margin:0" onclick="closeImg()">关闭</button>
    </div>
</div>

<div id="ctxMenu">
    <div class="ctx-item" onclick="renCur()">✎ 重命名</div>
    <div class="ctx-item" onclick="modDate()">📅 修改日期</div>
    <div class="ctx-item" onclick="modCost()">💰 修改金额</div>
    <div class="ctx-item" onclick="modImg()">🖼️ 修改图片</div>
    <div class="ctx-item ctx-del" onclick="delCur()">🗑️ 删除此项</div>
</div>
...
function mod(s){$('modal').style.display=s?'flex':'none';if(s){$('nDate').valueAsDate=new Date();tmpImg='';$('fTxt').innerText='📷 上传照片'}}
...
function rMain(){
    ...
    D.items.forEach((x,i)=>{
        ...
        el.innerHTML=`<img src="${x.img}" class="item-img" onclick="event.stopPropagation();showImg(${i})"><div class="item-info"><div class="item-n">${x.n} ${x.type=='car'?'🚗':''}</div><div class="item-d">${x.d} (${dys}天)</div></div>
        <div class="item-c"><div>¥${fmt(cost)}</div><div class="cost-day">¥${fmt(dayC)}/天</div></div>`;
...
function delCur(){if(ctxIdx>-1&&confirm('确认删除?')){D.items.splice(ctxIdx,1);sv()}hideCtx()}
function renCur(){if(ctxIdx<0)return;let x=D.items[ctxIdx],n=prompt('新名称',x.n);if(n){x.n=n;if(x.img.startsWith('data:'))x.img=txtImg(n);sv()}hideCtx()}
function modDate(){if(ctxIdx<0)return;let x=D.items[ctxIdx],d=prompt('新日期 (YYYY-MM-DD)',x.d);if(d){x.d=d;sv()}hideCtx()}
function modCost(){
    if(ctxIdx<0)return;let x=D.items[ctxIdx],old=x.type=='car'?x.p:x.c;
    let n=prompt(x.type=='car'?'修改购车费用':'修改金额',old);
...
function rCar(){
    ...
    $('fList').innerHTML=x.f.map((r,i,a)=>{
        let p=a[i+1], d=p ? r.o-p.o : 0; // p是上一次加油记录(里程更小)
        let thisL100 = 0;
        
        // 计算逻辑：如果里程差>0，则本次加油量 r.v 是为了补充这 d 公里的消耗
        if(d>0){
            td+=d; tv+=r.v; tf+=r.c;
            thisL100 = (r.v / d) * 100;
        }
        ...
        return `<div class="h-it f" onclick="edF(${r.id})">
            <div class="flex-bet"><span>${r.date}</span><b>¥${fmt(r.c)}</b></div>
            <div style="font-size:12px;color:#666">${infoLine}</div>
            <div class="del" onclick="event.stopPropagation();delSub('f',${r.id})">×</div>
        </div>`
    }).join('');
    ...
    let tm=0;
    $('mList').innerHTML=x.m.map(r=>{tm+=r.c;return `<div class="h-it m" onclick="edM(${r.id})"><div class="flex-bet"><span>${r.n}</span><b style="color:var(--o)">¥${fmt(r.c)}</b></div><div style="font-size:12px;color:#999">${r.date} (点击修改)</div><div class="del" onclick="event.stopPropagation();delSub('m',${r.id})">×</div></div>`}).join('');
    ...
    $('dCarDay').innerText=x.d ? '¥ '+fmt(tot/getDays(x.d)) : '--';
}
/* --- 修改结束 --- */

function addF(){let o=parseFloat($('cOdo').value),c=parseFloat($('cCost').value),v=parseFloat($('cVol').value),x=D.items[cur];if(!o||!c||!v)return alert('填全');if(x.f.length&&o<=x.f[0].o)return alert('里程错');x.f.unshift({id:Date.now(),date:new Date().toLocaleDateString(),o:o,c:c,v:v});rstF();sv();rCar()}
...
function edF(id){let r=D.items[cur].f.find(x=>x.id==id);$('cOdo').value=r.o;$('cCost').value=r.c;$('cVol').value=r.v;let b=$('btnF');b.innerText='保存修改';b.className='btn btn-g';b.onclick=()=>updF(id);$('tipF').style.display='block'}
function rstF(){$('cOdo').value='';$('cCost').value='';$('cVol').value='';let b=$('btnF');b.innerText='记一笔';b.className='btn btn-p';b.onclick=addF;$('tipF').style.display='none'}

function addM(){let n=$('mN').value,c=parseFloat($('mC').value),x=D.items[cur];if(!n||!c)return alert('填全');x.m.unshift({id:Date.now(),date:new Date().toLocaleDateString(),n:n,c:c});rstM();sv();rCar()}
function updM(id){let n=$('mN').value,c=parseFloat($('mC').value),x=D.items[cur].m.find(k=>k.id==id);if(x){x.n=n;x.c=c;rstM();sv();rCar()}}
function edM(id){let r=D.items[cur].m.find(x=>x.id==id);$('mN').value=r.n;$('mC').value=r.c;let b=$('btnM');b.innerText='保存修改';b.className='btn btn-g';b.onclick=()=>updM(id);$('tipM').style.display='block'}
function rstM(){$('mN').value='';$('mC').value='';let b=$('btnM');b.innerText='记保养';b.className='btn btn-o';b.onclick=addM;$('tipM').style.display='none'}

function delSub(t,id){if(confirm('删?')){let x=D.items[cur];if(t=='f')x.f=x.f.filter(r=>r.id!=id);else x.m=x.m.filter(r=>r.id!=id);sv();rCar()}}
function upCP(){let x=D.items[cur];x.p=parseFloat($('carP').value)||0;x.d=$('carD').value;sv();rCar()}
function expD(){let s=JSON.stringify(D);$('syncCode').value=btoa(encodeURIComponent(s));$('syncCode').select();document.execCommand('copy');alert('已复制')}
function impD(){let s=$('syncCode').value;if(!s)return;try{let j=decodeURIComponent(atob(s));if(confirm('覆盖?')){D=JSON.parse(j);sv();alert('成功');modSync(0)}}catch(e){alert('无效')}}
</script>
</body>
</html>
